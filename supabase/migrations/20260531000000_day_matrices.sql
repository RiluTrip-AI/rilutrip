-- The remote DB already has a `day_matrices` table from an earlier ad-hoc deploy
-- (optimize-route Edge Function v19), and the remote_baseline snapshot recreates
-- it with a stale shape (3-value matrix_source check, an owner-manage RLS policy).
-- Its cached rows are disposable, so drop and recreate it with this branch's
-- canonical schema (2-value check, viewer-select / editor-delete RLS). `if exists`
-- keeps this safe on a clean database where the table is absent.
drop table if exists public.day_matrices cascade;
...
create table public.day_matrices (
  ...
  constraint day_matrices_matrix_source_check
    check (matrix_source in ('google_routes_matrix', 'haversine_fallback'))
);
